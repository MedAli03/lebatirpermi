...
namespace J4xdemos\Component\Batirpermi\Administrator\View\Suivies;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		ToolbarHelper::title(Text::_('COM_BATIRPERMI_SUIVIES_TITLE'));
		...
		if ($user->authorise('core.edit.state', 'com_batirpermi'))
		{
			...
			$childBar->publish('suivies.publish')->listCheck(true);

			$childBar->unpublish('suivies.unpublish')->listCheck(true);

			$childBar->archive('suivies.archive')->listCheck(true);

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('suivies.trash')->listCheck(true);
			}
		}

		if ($this->state->get('filter.published') == -2 && $user->authorise('core.delete', 'com_batirpermi'))
		{
			$toolbar->delete('suivies.delete')
