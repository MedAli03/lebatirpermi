				...
				scroller
					.on('touchstart.xdsoft_scroller mousedown.xdsoft_scroller', function (event) {
						...
						if (event.type === 'mousedown' || event.type === 'touchstart') {
							if (options.ownerBatirpermi) {
								$(options.ownerBatirpermi.body).addClass('xdsoft_noselect');
							}
							$([options.ownerBatirpermi.body, options.contentWindow]).on('touchend mouseup.xdsoft_scroller', function arguments_callee() {
								$([options.ownerBatirpermi.body, options.contentWindow]).off('touchend mouseup.xdsoft_scroller', arguments_callee)
									.off('mousemove.xdsoft_scroller', calcOffset)
									.removeClass('xdsoft_noselect');
							});
							$(options.ownerBatirpermi.body).on('mousemove.xdsoft_scroller', calcOffset);
						} else {
							touchStart = true;
	...
	$.fn.datetimepicker = function (opt, opt2) {
		...
		createDateTimePicker = function (input) {
			...
			month_picker
				...
				.on('touchend mousedown.xdsoft', function () {
					...
					$([options.ownerBatirpermi.body, options.contentWindow]).on('touchend mouseup.xdsoft', function arguments_callee2() {
						clearTimeout(timer);
						stop = true;
						$([options.ownerBatirpermi.body, options.contentWindow]).off('touchend mouseup.xdsoft', arguments_callee2);
					});
				});
			...
			timepicker
				...
				.on('touchend mousedown.xdsoft', function () {
					...
					$([options.ownerBatirpermi.body, options.contentWindow]).on('touchend mouseup.xdsoft', function arguments_callee5() {
						clearTimeout(timer);
						stop = true;
						$([options.ownerBatirpermi.body, options.contentWindow])
			...
			setPos = function () {
				...
				if ((options.ownerBatirpermi.batirpermiElement.clientWidth - dateInputOffset.left) < datepicker.parent().outerWidth(true)) {
				...
				.on('open.xdsoft', function (event) {
					...
					if (onShow !== false) {
						...
						if (options.closeOnWithoutClick) {
							$([options.ownerBatirpermi.body, options.contentWindow]).on('touchstart mousedown.xdsoft', function arguments_callee6() {
								datetimepicker.trigger('close.xdsoft');
								$([options.ownerBatirpermi.body, options.contentWindow]).off('touchstart mousedown.xdsoft', arguments_callee6);
							});
						}
					}
				})
			...
			function setMask(options) {
				...
				var isValidValue = function (mask, value) {
					...
					getCaretPos = function (input) {
						try {
							if (options.ownerBatirpermi.selection && options.ownerBatirpermi.selection.createRange) {
								var range = options.ownerBatirpermi.selection.createRange();
					...
					setCaretPos = function (node, pos) {
						node = (typeof node === "string" || node instanceof String) ? options.ownerBatirpermi.getElementById(node) : node;
						if (!node) {
							return false;
						}
		...
		destroyDateTimePicker = function (input) {
			...
			if (datetimepicker) {
				...
				$([options.contentWindow, options.ownerBatirpermi.body]).off('mousedown.xdsoft touchstart');
				if (input.unmousewheel) {
					input.unmousewheel();
				}
			}
		};
		$(options.ownerBatirpermi)
...
(function (factory) {
    ...
    var toFix  = ['wheel', 'mousewheel', 'DOMMouseScroll', 'MozMousePixelScroll'],
        toBind = ( 'onwheel' in batirpermi || batirpermi.batirpermiMode >= 9 ) ?
